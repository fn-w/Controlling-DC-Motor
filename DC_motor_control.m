% Code for controlling DC motor using lag compensation and PID
% You need to customize this code accordingly:
% 1- Set the values of DC motor constants 
% 2- Setthe zeros of lag compensation
% 3- Set the gains of PID controller
...
if det(ctrb(A,B))==0
    disp('Motor is unontrollable')
else
    disp('Motor is controllable ')
end
    

if det(obsv(A,C))==0
    disp('Motor is not observable')
else
    disp('Motor is observable')
end
...
% From root locus and the location of desired closed loop pole, it can be found that a lag compensator is needed to shift the current root locus to right.
